...
/** Dev/demo fallback — same seeded scenario as before, no database required. */
export function buildMockNationalState(seed = 42): NationalState {
  return buildNationalState(buildDataset(seed));
}

export function buildNationalState(dataset: AegisDataset): NationalState {
