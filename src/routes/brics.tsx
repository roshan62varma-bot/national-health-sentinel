...
import { ErrorState, LoadingState, Panel, Stat } from "@/components/aegis/ui";
...
import { recordFlRounds } from "@/server-fn/aegis";
...
function BricsPanel() {
  ...
  // Seed local round history from the DB once on load; further user-run
  // rounds are appended locally (and persisted) rather than re-synced on
  // every poll, so an unsaved round in demo mode (no Supabase configured)
  // isn't wiped out by the next background refetch.
  useEffect(() => {
    if (state && rounds.length === 0) setRounds(state.dataset.flRounds);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state]);

  if (isLoading || !state) {
    return (
      <ConsoleShell breadcrumb="BRICS > Aggregate only" pulse={{ stockRisk: 0, bedRisk: 0, staffRisk: 0 }}>
        <LoadingState />
      </ConsoleShell>
    );
  }
  ...
  const runRound = () => {
    ...
    const newRounds = clientUpdates.map((u) => {
      ...
      return {
        ...
        aggregateAccuracyDelta:
          Math.round(
            (1 - blended.reduce((a, v, i) => a + Math.abs(v - (u.localDelta[i] ?? 0)), 0) / blended.length) * 100,
          ) / 100,
        timestamp: stamp,
      } satisfies FlRound;
    });
    setRounds((prev) => [...newRounds, ...prev]);
    recordFlRounds({ data: newRounds }).catch((err) => {
      console.error("Round ran locally but failed to persist to fl_rounds:", err);
    });
  };
